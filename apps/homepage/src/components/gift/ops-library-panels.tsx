...
export function CompleteModelsPanel({ models, csrfToken, onReload }: { models: OpsGiftModel[]; csrfToken: string; onReload: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    {!categoryMode ? <div className="grid gap-4 xl:grid-cols-2">{models.map((model) => <article key={model.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="grid sm:grid-cols-[180px_1fr]"><div className="grid min-h-44 place-items-center bg-slate-100">{model.preview_asset_id ? <img src={`/api/gift/ops/assets/${model.preview_asset_id}`} alt="" className="h-full max-h-52 w-full object-contain" /> : <FileBox className="h-10 w-10 text-slate-300" />}</div><div className="p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-black text-cyan-700">{categories.find((item) => item.slug === model.category)?.nameZh || model.category}</div><h3 className="mt-1 text-lg font-black">{model.title_zh}</h3><div className="mt-1 font-mono text-[11px] text-slate-400">#{model.sort_order} · {model.slug}</div></div><span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-black">{model.publication_status}</span></div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => openEdit(model)} className="rounded-md border px-3 py-2 text-xs font-black"><Edit3 className="mr-1 inline h-3.5 w-3.5" />编辑</button><label className="cursor-pointer rounded-md border px-3 py-2 text-xs font-black"><UploadCloud className="mr-1 inline h-3.5 w-3.5" />源模型<input type="file" accept=".stl,.obj,.3mf,.glb,.gltf" className="sr-only" onChange={(e) => void upload(model, 'model_file', e.target.files?.[0])} /></label><label className="cursor-pointer rounded-md border px-3 py-2 text-xs font-black"><UploadCloud className="mr-1 inline h-3.5 w-3.5" />主图<input type="file" multiple accept="image/png,image/jpeg,image/webp" className="sr-only" onChange={(e) => void uploadMany(model, 'model_preview', Array.from(e.target.files || []))} /></label></div><div className="mt-3 flex flex-wrap gap-2">{model.publication_status === 'draft' ? <button onClick={() => void setStatus(model, 'review')} className="rounded-md bg-amber-50 px-3 py-2 text-xs font-black text-amber-800">提交审核</button> : null}{model.publication_status !== 'published' ? <button onClick={() => void setStatus(model, 'published')} className="rounded-md bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-800">上架</button> : <button onClick={() => void setStatus(model, 'archived')} className="rounded-md bg-slate-100 px-3 py-2 text-xs font-black"><Archive className="mr-1 inline h-3.5 w-3.5" />下架归档</button>}</div></div></div><div className="border-t border-slate-100 p-4"><div className="mb-2 text-xs font-black text-slate-600">文件版本</div><div className="space-y-2">{(model.assets || []).map((asset) => <div key={asset.assetId} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-slate-50 px-3 py-2 text-xs"><div><span className="font-black">{assetRoleLabel(asset.role)} v{asset.version}</span><span className="ml-2 text-slate-500">{asset.filename} · {formatSize(asset.size)}</span>{asset.role === 'model_preview_3d' ? <span className="ml-2 text-slate-400">自动生成，不支持编辑</span> : asset.current ? <span className="ml-2 text-emerald-700">当前</span> : null}</div><div className="flex gap-2"><button type="button" onClick={() => setPreviewAsset(asset)} className="text-[#0b4f9c]" aria-label={`查看${asset.filename}`}><Eye className="h-4 w-4" /></button>{asset.role !== 'model_preview_3d' ? <button onClick={() => void removeAsset(model, asset)} className="text-red-600" aria-label={`删除${asset.filename}`}><Trash2 className="h-4 w-4" /></button> : null}</div></div>)}</div></div></article>)}</div> : null}
