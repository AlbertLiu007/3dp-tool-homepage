...
export function GiftModelModal({ language, model, onClose }: { language: 'zh' | 'en'; model: GeneratedGiftModel; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/55 p-2 backdrop-blur-sm sm:p-3 md:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-label={labels.title} className="flex h-[calc(100dvh-1rem)] max-h-[900px] w-full max-w-[1180px] min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl sm:h-[calc(100dvh-1.5rem)] md:h-[min(900px,calc(100dvh-3rem))]">
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain rounded-b-xl border-slate-200 bg-white">
          ...
          <div className="shrink-0 border-b border-slate-100 px-4 py-3"><div className="flex flex-wrap items-end justify-between gap-3"><div className="min-w-0 flex-1"><div className="text-xs font-bold text-slate-500">{labels.fileName}</div><div className="mt-1 truncate text-sm font-black text-slate-950">unionam-gift.{model.modelType}</div></div><div className="shrink-0 text-right"><div className="text-xs font-bold text-slate-500">{labels.fileSize}</div><div className="mt-1 text-sm font-black text-slate-950">{formatFileSize(fileSize)}</div></div></div></div>
